...
@commands_bp.route("/commands/power/on", methods=["POST"])
@login_required
def power_on():
    return jsonify({"command_id": f"cmd_{uuid.uuid4().hex[:8]}", "status": "completed", "action": "power_on", "message": "伺服致能開啟 (ON) 指令已成功送出！"})

@commands_bp.route("/commands/power/off", methods=["POST"])
@login_required
def power_off():
    return jsonify({"command_id": f"cmd_{uuid.uuid4().hex[:8]}", "status": "completed", "action": "power_off", "message": "伺服系統停能 (OFF) 指令已成功送出！"})
