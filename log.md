...
### 6. README 方式A寫死磁碟機代號 D: 修正

- **問題**：方式 A 說明「隨身碟複製即可直接執行」，指令卻寫死 `d:\20260708-Servo-motor-data-sheet\dist\...`，複製到別台電腦或不同槽別時該路徑不存在。
- **修正**：改為 `cd dist\AI_Servo_Platform` + `AI_Servo_Platform.exe` 相對路徑指令。

### 7. `project_report_gemini.md`（兩份，位於 `01_藍圖與設計規格/` 與 `專案報告/`）為舊版失真報告，已加註警告

- **問題**：使用者質疑「為什麼 Gemini 會給出造假數據」。追查後確認：
  1. 兩份文件的所有內部連結路徑指向 `d:/20260713/20260707-馬達專題-V4/...`——**舊版（V4）專案遺留文件**，從未針對現行 `20260708-Servo-motor-data-sheet` 專案的程式碼與模型重新產生或驗證。
  2. 文件「陸、聯調驗證指標與成果數據」章節（Recall 100%、F1-Score 95.81%、R²=0.9889、R²=1.0 等）皆可追溯到 `log.md` 歷史紀錄中 `phm_pipeline.py` 訓練管線的自報指標——**同一支腳本自己生成合成資料、自己訓練、自己驗證**，非獨立/真實資料驗證結果。
  3. 與本日稽核發現（見第 2 節）對照：同一顆 `演算法核心.pkl` 用現有 41 工況資料實測，`clf_stage`/`clf_trip` 準確率（26.7% / 83.7%）均低於多數類別對照組，且 158 特徵中 93 個為訓練期合成雜訊。
  4. **結論**：問題不在生成報告的 AI（Gemini），而在於餵給它的來源文件本身，把訓練管線的自我驗證指標當成已驗證的真實成果呈現。
- **修正**：在兩份文件開頭加註警告區塊，註明文件狀態為舊版遺留、指標為自報數據、對照本日稽核結果，並提醒不要再將此文件餵給任何 AI 當作專案現況簡報。
