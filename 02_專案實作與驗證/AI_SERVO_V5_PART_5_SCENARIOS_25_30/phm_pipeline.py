...
def chunk_generator(total_rows, chunk_size):
    proportions = {
        1: 0.20, 2: 0.05, 3: 0.05, 4: 0.05, 5: 0.05, 6: 0.05,
        23: 0.05, 25: 0.10, 26: 0.10, 27: 0.10, 28: 0.10, 29: 0.05, 30: 0.05
    }
    rows_generated = 0
...
def hybrid_classifier(df, ml_predictions, ml_confidences):
    ...
    for idx in range(len(df)):
        ...
        if (row["network_jitter_ms"] > 3.0 or row["ethercat_sync_error_us"] > 25.0) and row["vibration_rms_g"] < 0.35:
            if ml_predictions[idx] in ["resonance", "gain_instability", "following_error"]:
                final_preds.append("communication_loss_of_control")
                final_confs.append(1.0)
                continue
...
def map_root_cause_from_stage(pred_stage, row_idx, df):
    ...
    elif row["drive_temp_c"] > 85.0 and row["ethercat_packet_loss_pct"] < 1.8:
    ...
    if pred_stage == 0:
        ...
        if row["frequency_response_100hz_db"] > 22.0:
            return "resonance" if row["vibration_rms_g"] > 0.35 else "gain_instability"
        elif row["digital_twin_pos_residual"] > 100.0:
            return "brake_failure" if row["brake_status_bool"] == 1.0 else "following_error"
        else:
            return "progressive_failure"
...
def train_and_evaluate(parquet_path):
    ...
    for idx in range(len(df_test)):
        ...
        if row["plc_estop_active"] == 1.0:
            true_root_causes.append("emergency_stop")
        elif row["encoder_error_count"] > 1000.0:
            true_root_causes.append("encoder_signal_loss")
        elif row["encoder_error_count"] > 50.0:
            true_root_causes.append("encoder_noise")
        elif row["encoder_drift_pulse"] > 100.0:
            true_root_causes.append("encoder_drift")
        elif row["ethercat_packet_loss_pct"] > 2.0 and row["following_error_abs_pulse"] > 100.0:
            true_root_causes.append("communication_loss_of_control")
        elif row["brake_status_bool"] == 1.0 and row["digital_twin_pos_residual"] > 150.0:
            true_root_causes.append("brake_failure")
        elif row["bearing_bpfo_amp"] > 1.2 or row["bearing_bpfi_amp"] > 1.2:
            true_root_causes.append("progressive_failure")
        elif row["motor_temp_c"] > 90.0 and row["ethercat_packet_loss_pct"] > 1.8:
            true_root_causes.append("combined_fault")
        elif row["motor_temp_c"] > 90.0:
            true_root_causes.append("motor_over_temp")
        elif row["drive_temp_c"] > 85.0:
            true_root_causes.append("drive_over_temp")
        elif row["frequency_response_100hz_db"] > 25.0:
            true_root_causes.append("resonance" if row["vibration_rms_g"] > 0.35 else "gain_instability")
        else:
            true_root_causes.append("normal")
