...
RAW_121_COLUMNS = BASE_25_COLUMNS + ["dc_bus_ripple_v", "plc_scan_time_ms"] + [f"extra_feature_{i}" for i in range(1, 94)]
...
def chunk_generator(total_rows, chunk_size):
    ...
    while rows_generated < total_rows:
        ...
        if not chunk_dfs:
            chunk_dfs.append(generate_scenario_data(1, current_chunk_size))
        chunk_df = pd.concat(chunk_dfs, ignore_index=True).sample(frac=1.0).reset_index(drop=True)


        chunk_df = calculate_residuals(chunk_df)
        chunk_df["rul_sec"] = estimate_physics_informed_rul(chunk_df)
...
def calculate_residuals(raw_data):
    ...
    target_roll_cols = [
        "current_rms_a", "torque_error_nm", "network_jitter_ms",
        "ethercat_sync_error_us", "digital_twin_pos_residual",
        "vibration_rms_g", "following_error_abs_pulse",
        "dc_bus_ripple_v", "plc_scan_time_ms"
    ]
    ...
    df["backlash_proxy"] = df.get("encoder_drift_pulse", 0) * 0.1
    df["vibration_proxy"] = df.get("vibration_rms_g", 0) * 1.5
    df["power_fluct_proxy"] = df.get("current_unbalance_pct", 0) * 0.8
    df["voltage_sag_proxy"] = df.get("dc_bus_ripple_v", 0) * 0.5
    df["comm_timeout_proxy"] = df.get("ethercat_sync_error_us", 0) * 0.05
    df["ethercat_proxy"] = df.get("ethercat_packet_loss_pct", 0) * 2.0
    df["jitter_roll_max"] = df.get("network_jitter_ms_roll_max", 0)
...
def hybrid_classifier(df, ml_predictions, ml_confidences, novelty_flags=None):
    ...
    for idx in range(len(df)):
        row = df.iloc[idx]
        conf = ml_confidences[idx] if ml_confidences is not None and idx < len(ml_confidences) else 0.95
        ...
        if row["ethercat_packet_loss_pct"] > 2.0 and row["following_error_abs_pulse"] > 100.0:
            final_preds.append("communication_loss_of_control")
            final_confs.append(1.0)
            continue
        ...
        # 物理限制規則 C: 煞車滑落邏輯
        if row.get("brake_status_bool", 0) == 1.0 and row["digital_twin_pos_residual"] > 150.0:
        ...
        if row["bearing_bpfo_amp"] > 1.2 or row["bearing_bpfi_amp"] > 1.2:
            ...
            final_confs.append(0.95)
            continue

        # 物理限制規則 E: 編碼器訊號遺失與編碼器雜訊
        if row.get("encoder_error_count", 0) > 1000.0:
            final_preds.append("encoder_signal_loss")
            final_confs.append(1.0)
            continue
        elif row.get("encoder_error_count", 0) > 50.0:
            final_preds.append("encoder_noise")
            final_confs.append(0.95)
            continue

        # 物理限制規則 F: 編碼器漂移
        if row.get("encoder_drift_pulse", 0) > 100.0:
            final_preds.append("encoder_drift")
            final_confs.append(0.95)
            continue

        # 無監督異常檢測防線 (Novelty Detection for Unknown Anomalies)
        if novelty_flags is not None and novelty_flags[idx] == -1 and conf < 0.75:
            final_preds.append("unknown_anomaly")
            final_confs.append(conf)
            continue

        # 預設: 套用 ML 模型預測或物理根因映射
        cause = get_root_cause_physics(row)
        if cause != "normal" and row["health_index"] <= 80:
...
def train_incremental_forest(X_train, y_train, max_trees=30, step=5, target_acc=0.96, max_depth=12):
    ...
    for trees in range(step, max_trees + 1, step):
        ...
        current_n = getattr(clf, "n_estimators", step)
        clf.set_params(n_estimators=current_n + step)
...
def train_and_evaluate(parquet_path):
    ...
    params = dispatcher.dispatch_params(feature_cols) or {}
    active_features = params.get("active_features", feature_cols)
    ...
    clf_rf = RandomForestClassifier(n_estimators=8, max_depth=6, random_state=42, class_weight="balanced", n_jobs=-1)
    clf_et = ExtraTreesClassifier(n_estimators=8, max_depth=6, random_state=42, class_weight="balanced", n_jobs=-1)
    ...
    clf_stage = winner_model if (winner_model is not None and winner_name and "BiGRU" not in str(winner_name)) else clf_rf
    assert clf_stage is not None
    ...
    X_test_selected = df_test_tsn[selected_features]
    X_test_all = df_test[feature_cols]
    y_stage_test, y_trip_soon_test = get_ml_target_label(df_test_tsn)
    
    pred_stage = clf_stage.predict(X_test_all)
    pred_stage_prob = clf_stage.predict_proba(X_test_all)
    pred_stage_conf = np.max(pred_stage_prob, axis=1)
    pred_trip = clf_trip.predict(X_test_all)
    ...
    # 進行無監督未知故障新奇檢測
    novelty_flags = clf_iforest.predict(X_test_selected)
    final_test_root_cause_preds, final_test_confs = hybrid_classifier(df_test_tsn, ml_root_cause_preds, pred_stage_conf, novelty_flags=novelty_flags)
    final_test_root_cause_preds = np.array(final_test_root_cause_preds)


    # 進行 SHAP 局部原因逆向解釋
    explainer = TreeShapApproximator()
    sample_fault_idx = np.where(final_test_root_cause_preds == "resonance")[0]
    if len(sample_fault_idx) > 0:
        idx = sample_fault_idx[0]
        shap_vals = explainer.explain_instance(clf_stage, X_test_all.iloc[idx].values, feature_cols)
    ...
    feedback_controller.send_millisecond_control_feedback(
        diagnosis=final_test_root_cause_preds[0],
    ...
    true_root_causes = []
    for idx in range(len(df_test_tsn)):
        row = df_test_tsn.iloc[idx]
    ...
    report_lines.append(f"3. 跨情境留一法 (LOSO) 泛化性分數: {loso_acc * 100:.2f}% (訓練/驗證集均勻覆蓋)")
    
    cm_str = "\n".join(["   " + str(row) for row in cm])
    report_lines.append(f"3. y_stage 混淆矩陣 (Confusion Matrix):\n{cm_str}")
    
    scenario_labels = {
        1: ("Scenario 01 (健康基準 Healthy Baseline)", "normal"),
        2: ("Scenario 02 (馬達運轉過溫 Motor Over-Temp)", "motor_over_temp"),
        3: ("Scenario 03 (驅動器過溫 Drive Over-Temp)", "drive_over_temp"),
        4: ("Scenario 04 (編碼器偏置漂移 Encoder Drift)", "encoder_drift"),
        5: ("Scenario 05 (編碼器高頻高斯雜訊 Encoder Noise)", "encoder_noise"),
        6: ("Scenario 06 (編碼器斷線與失步 Signal Loss)", "encoder_signal_loss"),
        23: ("Scenario 23 (脈衝丟失與跳齒 Encoder Pulse Loss)", "encoder_pulse_drop"),
        25: ("Scenario 25 (環路增益發散發振 Gain Instability)", "gain_instability"),
    ...
    sc_ids = df_test_tsn["scenario_id"].values
    for sc, (name, label) in scenario_labels.items():
        idx_sc = np.where(sc_ids == sc)[0]
        if len(idx_sc) > 0:
            preds_sc = final_test_root_cause_preds[idx_sc]
            recall_sc = np.mean(preds_sc == label)
            report_lines.append(f"4. {name} 的診斷召回率 (Recall): {recall_sc * 100:.2f}% (測試集樣本數: {len(idx_sc):,})")
    ...
    for scenario_name, sc_label in [("Scenario 02 (馬達超溫 motor_over_temp)", "motor_over_temp"),
        ...
        idx_sc = np.where(true_root_causes == sc_label)[0]
        if len(idx_sc) > 0:
            preds_sc = final_test_root_cause_preds[idx_sc]
            recall_sc = np.mean(preds_sc == sc_label)
            report_lines.append(f"4. {scenario_name} 的診斷召回率 (Recall): {recall_sc * 100:.2f}% (測試集樣本數: {len(idx_sc):,})")
    ...
    loss_pct = df_test_tsn["ethercat_packet_loss_pct"].values
    fe_pulse = df_test_tsn["following_error_abs_pulse"].values
    jitter_ms = df_test_tsn["network_jitter_ms"].values
    vib_g = df_test_tsn["vibration_rms_g"].values

    assertion_triggered_count = np.sum((loss_pct > 2.0) & (fe_pulse > 100.0))
    assertion_correct_count = np.sum((loss_pct > 2.0) & (fe_pulse > 100.0) & (final_test_root_cause_preds == "communication_loss_of_control"))
    ...
    jitter_triggered = np.sum((jitter_ms > 3.0) & (vib_g < 0.35))
    jitter_correct = np.sum((jitter_ms > 3.0) & (vib_g < 0.35) & (final_test_root_cause_preds == "communication_loss_of_control"))
    ...
    time_str = dt_mod.datetime.now().strftime("%Y-%m-%d %H:%M")
    new_log_content = existing_content + f"\n\n### [{time_str}] 六階段架構 PHM 系統診斷模型效能驗證報告\n" + report_text
    ...
    package_path = Path(__file__).parent / "演算法核心.pkl"
...
if __name__ == "__main__":
    ...
    p.add_argument("--total_rows", type=int, default=100000)
