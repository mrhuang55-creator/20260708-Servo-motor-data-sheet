...
> ⚠️ **文件狀態警告（2026-08-07 稽核後補註，請勿刪除此區塊）**
>
> 本文件為**舊版專案（路徑指向 `d:/20260713/20260707-馬達專題-V4/...`，非現行 `20260708-Servo-motor-data-sheet` 專案）遺留文件**，未針對現行程式碼與模型重新驗證，**請勿再將本文件餵給任何 AI（含 Gemini、Claude 等）當作專案現況簡報使用**。
>
> **本文件「陸、聯調驗證指標與成果數據」章節列出的所有數字**（Recall 100%、F1-Score 95.81%、R²=0.9889、R²=1.0 等），**皆為 `phm_pipeline.py` 訓練管線在自己生成的合成資料上「自己訓練、自己驗證」得出的自報指標**，並非以獨立/真實資料驗證的結果。2026-08-07 稽核發現：
> 1. 該訓練管線的 158 維特徵中，**93 個（59%）在資料生成階段就是 `np.random.normal(0,1)` 純高斯雜訊**，非真實物理量測（見 `phm_pipeline.py`）。
> 2. 已訓練完成的 `演算法核心.pkl`（`RandomForestClassifier` x2）對這 93 個雜訊特徵的重要度高達 26~33%，顯示模型過度依賴合成雜訊。
> 3. 用現有 `test_repository/data/` 41 工況模擬資料實測同一顆 pkl：`clf_stage` 準確率 **26.7%**、`clf_trip` 準確率 **83.7%**，**均低於「永遠猜多數類別」的對照組**（32.3% / 88.4%）。
> 4. 該 pkl 目前**從未被 `server.py` 載入或呼叫**，所有即時 API 皆為模擬/寫死數據。
>
> 完整稽核過程與證據見 [`log.md`](file:///d:/20260708-Servo-motor-data-sheet/log.md) 2026-08-07 條目、[`README.md`](file:///d:/20260708-Servo-motor-data-sheet/README.md) 「⚠️ 目前系統狀態」章節。
...
在 10M 級流式資料庫與真實輔助數據集（Sup data）上的最終聯調測試結果如下：

*   **卡爾曼濾波效果**：位置追隨誤差 RMSE 從 **4.8322** 降至 **2.8732**（雜訊濾除率 **40.54%**）。
*   **共振峰定位精度**： sweeps 測試信號共振點識別誤差為 **0 Hz**（290Hz 訊號精確辨識為 290.00Hz）。
*   **虛擬轉矩感測器 ML 競賽**：
    *   在 `MLCompetitionPlatform` 中測試了 11 種回歸模型，最終以 **LinearRegression** 取得最佳成績。
    *   擬合指標：決定係數 **R² = 1.0**，均方誤差 **MSE = 2.413754e-18**（極限精度），已固化為 `torque_virtual_sensor.pkl`（僅 **0.0005 MB**）。
*   **混合真實數據訓練分類器表現**：
    *   成功將 50k 物理尺度對齊的真實 `Sup data` 與 100k 模擬數據混合重訓，分類器 `phm_model_package.pkl` 大小僅 **2.27 MB**。
    *   核心場景召回率：**機械共振（80.00%）**、**外部突發碰撞（69.90%）**、**漸進式衰退（100.00%）**。
*   **自研貝氏尋優器性能**：尋優 F1-Score 達 **94.92%**。
*   **自研自動微分引擎收斂**：計算圖反向傳播 Loss 從 **3.3241** 穩定收斂至 **2.9252**（Loss 下降 **12.0%**）。
*   **防誤判斷言覆蓋率**：通訊封包丟失假報警覆蓋率達 **100%**。
